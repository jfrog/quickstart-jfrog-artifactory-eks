...
=== Add the license keys to AWS Secrets Manager

. Open AWS Secrets Manager in the same Region in which you deploy the Quick Start.
. Choose Store a new secret.
. Choose Other type of secret.
. For the secret key value, create seven rows for the Artifactory licenses.
. Key names should be as follows, with the key values being Artifactory license keys (see <<secrets_manager_licenses>>)):
.. `ArtifactoryLicense1`
.. `ArtifactoryLicense2`
.. `ArtifactoryLicense3`
.. `ArtifactoryLicense4`
...
:xrefstyle: short
[#secrets_manager_licenses]
.Secrets Manager key-value page
[link=../{quickstart-project-name}/images/secrets_manager_licenses.png]
image::../images/secrets_manager_licenses.png[image_placeholder]
...
. Choose Store a new secret.
. Choose Other type of secret.
. For the secret key value, create seven rows for the certificate information.
. Key names should be as follows, with the key values being certificate details (see <<secrets_manager_certificates>>)):
.. `Certificate`
.. `CertificateKey`
.. `CertificateDomain`

:xrefstyle: short
[#secrets_manager_certificates]
.Secrets Manager key-value page
